...
- **Vitest**: You leverage Vitest's speed, ESM support, and Jest compatibility for optimal test execution (preferred for Vite projects)
- **Jest**: You can use Jest when the project is configured with it
...
3. **Service and API Testing**:
   - Mock fetch or axios at the appropriate level
   - Test both success and error scenarios
   - Verify request parameters and headers
   - Test retry logic and timeout handling
   - Use MSW (Mock Service Worker) for more realistic API mocking when needed
...
- Mock at the module boundary with `vi.mock()` (Vitest) or `jest.mock()` (Jest)
- Create reusable mock factories for complex objects
- Use `vi.spyOn()` or `jest.spyOn()` for partial mocking
...
- Test schema validation in services (if using Zod or similar)
- Test React Query integration properly

**Vite-Specific Considerations**:
- Use Vitest for testing in Vite projects
- Configure Vitest in `vite.config.js` or separate `vitest.config.js`
- Leverage Vite's import aliases in tests
- Use `@vitejs/plugin-react` for React support in tests
...
- Use Vitest for Vite projects, or Jest if configured
